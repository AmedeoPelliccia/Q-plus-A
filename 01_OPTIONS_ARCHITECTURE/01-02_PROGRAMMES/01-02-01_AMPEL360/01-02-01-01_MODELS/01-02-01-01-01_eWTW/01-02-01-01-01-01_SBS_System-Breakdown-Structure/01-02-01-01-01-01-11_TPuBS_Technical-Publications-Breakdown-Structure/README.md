...
side: PUB
organization: information-centric (single S-ATLAS instance; DMRL per node; PMCs as publication views)
...
status: baseline
version: "3.0"
supersedes: "TPuBS README v2.0 (manual-first; S-ATLAS replicated under each PMC)"
---

# eWTW · TPuBS — Technical Publications Breakdown Structure

The **root** of the eWTW model's technical information, and the `-11` (PUB) member of the SBS. The TPuBS is organized **information-centric**: a single canonical S-ATLAS information architecture with a **DMRL per node**, a common Data Module pool, and publications defined as **Publication Module compositions** that reference — never own — the Data Modules.

> **Correction (v3.0).** v2.0 organized the TPuBS manual-first, replicating the S-ATLAS/ATA breakdown under every PMC. That recreated the traditional book paradigm in which information is implicitly "owned" by the manual. Per the CSDB/S1000D principle, a Data Module is a standalone, reusable information unit identified by its DMC and managed in the CSDB; the manual is not the primary container of information but a **controlled composition of the common information pool**.

## Doctrine

> **The eWTW TPuBS is information-centric and organized by S-ATLAS. Every S-ATLAS node declares, through a local DMRL, the InfoCodes and Data Modules it requires. Manuals are not autonomous containers of information but publication views configured through Publication Modules that select, order and publish only the Data Modules applicable to their purpose.**

Equivalently:

> **S-ATLAS determines where information semantically belongs; the DMRL determines which Data Modules must exist; the Publication Modules determine in which publications, and in which order, those Data Modules are presented.**
...
- [3. DMRL per S-ATLAS Node](#3-dmrl-per-g-atlas-node)
- [4. Publication Modules Separated from the S-ATLAS Tree](#4-publication-modules-separated-from-the-g-atlas-tree)
- [5. Not Every Node Needs Every Manual](#5-not-every-node-needs-every-manual)
- [6. S-ATLAS Is the Allocation Backbone, Not a Mandatory TOC](#6-g-atlas-is-the-allocation-backbone-not-a-mandatory-toc)
...
├── 01_INFORMATION-ARCHITECTURE/     ← single canonical S-ATLAS instance + common DM pool
│   ├── S-ATLAS_000-099/
│   ├── EPTA_400-499/
│   └── AMTA_500-599/
...
| `01_INFORMATION-ARCHITECTURE` | Canonical classification by S-ATLAS/SNS; local DMRL per node; common DM pool |
...
```text
eWTW product/model → S-ATLAS/SNS → information nodes → node DMRLs
   → common Data Modules → Publication Modules → AMM, FIM, SDS, IPC, FCOM, …
```

---

## 3. DMRL per S-ATLAS Node
...
```text
one S-ATLAS node → several DM requirements / InfoCodes
...
## 4. Publication Modules Separated from the S-ATLAS Tree
...
## 6. S-ATLAS Is the Allocation Backbone, Not a Mandatory TOC

AMM, SDS, FIM and IPC can largely follow the ATA/S-ATLAS breakdown (`chapter → section → subject`). Other publications follow purpose-specific editorial axes while keeping the S-ATLAS link through DM references:
...
Not: `S-ATLAS = identical mandatory table of contents of every manual`.
...
Inherits **DEGF v1.0**; governed across **LC-A … LC-N**; bound by **No-AAA** and **SSOT+PUB**. Publication types authorized by the SSOT DTD register; terminology by **S-ATLAS-NORM-TERM-001**. Cross-cutting CSDB rules live in `00_TPUBS-GOVERNANCE/`. Owner **Q-DATAGOV**.
...
Last.MarkedDown:
  ...
  revision: v3.0 — information-centric; single S-ATLAS instance; DMRL per node; PMCs as publication views
  supersedes: v2.0 (manual-first; S-ATLAS replicated under each PMC)
  version: "3.0"
