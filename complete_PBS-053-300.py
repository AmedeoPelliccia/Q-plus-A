#!/usr/bin/env python3
# =============================================================================
# complete_PBS-053-300.py  (1.3.1) - controlled migration pass
# Depth Pass 1 - section eWTW-PBS-053-300 (Aft Fuselage): adds missing PN nodes per
...
PROV  = f"# Amended {TODAY} - complete_PBS-053-300.py (1.3.1)"
GEN   = f"# Generated {TODAY} - complete_PBS-053-300.py (1.3.1)"

# ---------------------------------------------------------------- target spec
# (station csn, station folder, [(item, parent_item|None, nomenclature, role, qty, uom, mb)])
SPEC = {
 "533001": ("eWTW-PBS-053-300-010_Rear-Fuselage-Zone-General", [
   ("000", None,  "STRUCTURE-REAR-FUSELAGE-ZONE",           "assembly",   1,"EA","make"),
   ("010", "000", "SPLICE-RING-AFT-TO-TAILCONE-JOIN",       "set",        1,"SET","make"),
   ...
   ("030", "000", "SUPPORT-SYSTEMS-RUN-SET",                "set",        1,"SET","make"),
   ("031", "030", "BRACKET-SYSTEMS-RUN",                    "leaf",      12,"EA","make"),
   ("040", "000", "SURROUND-OUTFLOW-VALVE",                 "set",        1,"SET","make"),
   ("041", "040", "DOUBLER-OUTFLOW-VALVE-CUTOUT",           "leaf",       1,"EA","make"),
   ("042", "040", "FRAME-SURROUND-OUTFLOW-VALVE",           "leaf",       1,"EA","make"),
   ("050", "000", "PROVISION-NDT-ACCESS-JOIN-SET",          "set",        1,"SET","make"),
   ("051", "050", "PLUG-INSPECTION-JOIN",                   "leaf",      12,"EA","buy"),
 ]),
 "533002": ("eWTW-PBS-053-300-020_Rear-Fuselage-Adjustable-Rods", [
   ("000", None,  "SET-ADJUSTABLE-RODS-REAR",               "assembly",   1,"EA","make"),
   ("010", "000", "ROD-ADJUSTABLE",                         "set",        1,"SET","make"),
   ("011", "010", "ROD-ADJUSTABLE-LH",                      "constituent",1,"EA","make"),
   ("012", "010", "ROD-ADJUSTABLE-RH",                      "constituent",1,"EA","make"),
   ("020", "000", "FITTING-ROD-END-SET",                    "set",        1,"SET","buy"),
   ("021", "020", "ROD-END-SPHERICAL",                      "leaf",       4,"EA","buy"),
   ("022", "020", "FITTING-ROD-END-CLEVIS",                 "leaf",       4,"EA","make"),
   ("030", "000", "BRACKET-ROD-SUPPORT-SET",                "set",        1,"SET","make"),
   ("031", "030", "BRACKET-ROD-SUPPORT-UPPER",              "leaf",       2,"EA","make"),
   ("032", "030", "BRACKET-ROD-SUPPORT-LOWER",              "leaf",       2,"EA","make"),
   ("040", "000", "PROVISION-ROD-ADJUSTMENT-ACCESS-SET",    "set",        1,"SET","make"),
   ("041", "040", "PLUG-ACCESS-ADJUSTMENT",                 "leaf",       2,"EA","buy"),
 ]),
 "533003": ("eWTW-PBS-053-300-030_Tail-Bumper", [
   ("000", None,  "STRUCTURE-TAIL-BUMPER",                  "assembly",   1,"EA","make"),
   ("010", "000", "SHOE-TAIL-BUMPER",                       "set",        1,"SET","make"),
   ("011", "010", "INSERT-WEAR-SHOE",                       "leaf",       1,"EA","buy"),
   ("020", "000", "ABSORBER-TAIL-BUMPER",                   "set",        1,"SET","buy"),
   ("021", "020", "CARTRIDGE-ABSORBER-CRUSHABLE",           "leaf",       1,"EA","buy"),
   ("030", "000", "FITTING-BUMPER-ATTACH",                  "set",        1,"SET","make"),
   ("031", "030", "PIN-BUMPER-ATTACH",                      "leaf",       2,"EA","buy"),
   ("032", "030", "BUSHING-BUMPER-ATTACH",                  "leaf",       2,"EA","buy"),
   ("040", "000", "PROVISION-TAILSTRIKE-INDICATOR-SET",     "set",        1,"SET","make"),
   ("041", "040", "BRACKET-INDICATOR-MOUNT",                "leaf",       1,"EA","make"),
 ]),
 "533004": ("eWTW-PBS-053-300-040_Rear-Fuselage-External-Access-Panels", [
   ("000", None,  "SET-ACCESS-PANELS-REAR",                 "assembly",   1,"EA","make"),
   ("010", "000", "PANEL-ACCESS-EMPENNAGE-SERVICE-SET",     "set",        1,"SET","make"),
   ("011", "010", "PANEL-EMPENNAGE-SERVICE",                "leaf",       4,"EA","make"),
   ("020", "000", "PANEL-ACCESS-BULK-SET",                  "set",        1,"SET","make"),
   ("021", "020", "PANEL-ACCESS-BULK-CARGO",                "leaf",       2,"EA","make"),
   ("022", "020", "SILL-PANEL-BULK",                        "leaf",       2,"EA","make"),
   ("030", "000", "SURROUND-PANEL-CUTOUT-SET",              "set",        1,"SET","make"),
   ("031", "030", "FRAME-CUTOUT-REINFORCED",                "leaf",       6,"EA","make"),
   ("040", "000", "STANDARD-PANEL-HARDWARE-INSTALLATION",   "installation",1,"SET","make",
     [("EWTW-530003-011",12,"EA"),("EWTW-530003-021",6,"EA"),("EWTW-530003-031",1,"AR")]),
   ("050", "000", "STANDARD-PANEL-INSTALLATION",            "installation",1,"SET","make",
     [("EWTW-530003-041",2,"EA"),("EWTW-530003-042",4,"EA")]),
 ]),
}

REALIZES = {"533001":"053-300-100","533002":"053-300-200",
            "533003":"053-300-300","533004":"053-300-400"}
BOUNDARY = {
 "533001": "Zone-general structure of the rear fuselage: the aft-to-tailcone production join, systems-run structural supports, the outflow-valve structural surround and zone NDT provisions. The rear pressure bulkhead (053-800-020) and the stabilizer attach fittings (053-800-070) are received here and owned there; the outflow valve is ATA 021; empennage structure is ATA 055.",
 "533002": "Rear-fuselage adjustable rods: rods (LH/RH), rod-end fittings, support brackets and adjustment-access provisions. Rigging values are AMM data; the rods are structure.",
 "533003": "Tail-bumper structure: wear shoe with replaceable insert, crushable absorber cartridge, attach fittings, and the structural provision for the tail-strike indicator. Indicator/sensing-system ownership remains pending configuration ruling.",
 "533004": "Rear external access panels: zone-unique empennage-service and bulk panels are local PNs; standard panels and hardware are catalog identities from 053-000-030, carried as references with quantities (CM-001 C1.2).",
}

MIGRATION_LOCKED = set()        # no migration acts in this section

SCRIPT_NAME = "complete_PBS-053-300.py"
...
assert 'else "# eWTW-PBS-053-300-000' in _src, "SELF-CHECK: section README fallback belongs to another section"
for _foreign in ("053-000-000_General","053-100-000_Nose","053-200-000_Center","053-300-000_Aft"):
    if not _foreign.startswith("053-300-000"):
...
SEEDS = {
 "533001": [
  {"icd":"ICD-EWTW-533001-538002","space":"pbs-local","counterpart":"eWTW-PBS-053-800-020","counterpartCsn":"538002",
   "taxonomyRef":"053-800-200","item":"rear pressure bulkhead - rear zone structure receives, 053-800-020 owns (zone-level joint; load path not yet demonstrated)",
   "carriedBy":["EWTW-533001-000"]},
  {"icd":"ICD-EWTW-533001-534001","space":"pbs-local","counterpart":"eWTW-PBS-053-400-010","counterpartCsn":"534001",
   "taxonomyRef":"053-400-100","item":"aft-to-tailcone production join",
   "carriedBy":["EWTW-533001-010","EWTW-533001-020"]},
  {"icd":"ICD-EWTW-533001-021","space":"taxonomy","counterpart":"021",
   "item":"outflow valve - structural surround here, valve and function ATA 021",
   "carriedBy":["EWTW-533001-040","EWTW-533001-041","EWTW-533001-042"]},
 ],
 "533003": [],   # tail-strike owning system pending ruling (taxonomy crossref empty) - no seed
}
...
def norm(s):  # comparison ignoring provenance lines entirely
    ...
    s = re.sub(r"^_Generated \d{4}-\d{2}-\d{2} - complete_PBS-053-300\.py \([^)]+\)_\n?", "", s, flags=re.M)
    return re.sub(r"\n{2,}", "\n", s).strip()
...
def main():
    ...
    a = ap.parse_args()
    root = Path(a.root)
    hits = list(root.rglob("eWTW-PBS-053-300-000_Aft-Fuselage-Structure"))
    if not hits:
        print("section folder eWTW-PBS-053-300-000_Aft-Fuselage-Structure not found under", root); sys.exit(1)
    sec = hits[0]
    W = a.apply
    ...
    for csn, (sta_folder, rows) in SPEC.items():
        ...
        for it, par, nom, role, qty, uom, mb, *refs in rows:
            ...
            if py.exists():
                ...
                for k in ("pn","csn","item","parentAssembly"):
                    ...
                    if not any("complete_PBS-053-300.py" in l and l.startswith("# Amended") for l in header_lines):
        ...
        if sy.exists():
            ...
            if not any("complete_PBS-053-300.py" in l for l in hdr):
                hdr.append(PROV)
        ...
        block = ("<!-- BOM:BEGIN (derived - regenerated by complete_PBS-053-300.py; do not edit) -->\n"
        ...
        reg = {"register": {"station": f"eWTW-PBS-053-300-{sta_folder.split('_')[0][-3:]}",
    ...
    blk = ("<!-- SECTION:BEGIN (derived - regenerated by complete_PBS-053-300.py; do not edit) -->\n"
           ...
           + "<!-- SECTION:END -->")
    t0 = sec_rd.read_text() if sec_rd.exists() else "# eWTW-PBS-053-300-000 - Aft Fuselage Structure\n"
    ...
    out = GEN + "\n" + yaml.safe_dump({"register": {"section": "eWTW-PBS-053-300",
    ...
    tax = None
    for cand in root.rglob("053-300_Aft-Fuselage-Structure"):
    ...
    contam = sum(1 for f in list(sec.rglob("part-register.yaml"))+list(sec.rglob("pbs-item-register.yaml")) if re.search(r"eWTW-PBS-053-(?!300-)\d00-\d", f.read_text()))
